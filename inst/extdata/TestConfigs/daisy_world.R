...
end_of_timestep_observer <- function(data, vars, config) {
  ...
  modifiers <- sapply(data$space_modifiers, function(mod) {
    mod
  })
...
create_ancestor_species <- function(space, config) {
  ...
  black_daisy <- create_species(
    initial_cells = initial_sites[1:10],
    config = config
  )
  ...
  white_daisy <- create_species(
    initial_cells = initial_sites[11:20],
    config = config
  )
...
apply_ecology <- function(abundance, traits, ecological_states, local_environment, config) {
  if (length(abundance) > 1) {
    ...
    must_survive <- sample(
      c(1, 2, "none"),
      1,
      prob = c(only_black, only_white, extreme_deathly_death_from_hell)
    )

    abundance <- sapply(names(abundance), function(sp) {
      0
    })
    ...
    survival_change <- survival_baseline^(temp_distance /
      traits[, "thermal_tolerance"])

    abundance[1] <- sample(
      c(1, 0),
      1,
      prob = c(survival_change, 1 - survival_change)
    )
  }
...
modify_space <- list(
  ...
  apply_modifiers = function(space, modifiers) {
    ...
    cat(
      ...
      mean(space$environment[, "temperature"]),
      "\n"
    )
    space$environment[, "temperature"] <- space$environment[, "temperature"] +
      (energy_delta * 1)
    cat(
