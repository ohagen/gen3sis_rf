...
apply_ecology <- function(abundance, traits, ecological_states, local_environment, config) {
  ...
  abundance[!survive] <- 0
  abundance <- (( 1-abs( traits[, "temp"] - local_environment[, "temp"]))*abundance_scale)*as.numeric(survive)
  #abundance threshold
  abundance[abundance<abundance_threshold] <- 0
  k <- (
    (
      local_environment[,"area"] * 
        (local_environment[,"arid"]+0.1) * 
...
config_object$gen3sis$ecology$apply_ecology <- function(abundance, traits, ecological_states, 
                                                        local_environment, config, abundance_scale = 10, abundance_threshold = 8) {
  #abundance threshold
  abundance <- as.numeric(!abundance<abundance_threshold)
  abundance <- (( 1-abs( traits[, "temp"] - local_environment[, "temp"]))*abundance_scale)*abundance
  #abundance threshold
  abundance[abundance<abundance_threshold] <- 0
  return(abundance)
}
