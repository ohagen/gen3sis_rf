...
test_that("prepare_directories: Error on missing config file", {
  # With no config
  expect_error(prepare_directories(), "no config file provided!", fixed = TRUE)

  # With a config file that does not exist
  expect_error(
    prepare_directories(config_file = "not_exisiting.R"),
...
test_that("derive input/output directories from config path", {
  expected_directories <- list(
    ...
    output_plots = file.path(
      gsub("[Cc]onfig", "output", base_dir),
      "TestConfig",
      "plots"
    )
  )
...
test_that("prepare_directories: config, input-, and output- directory given", {
  ...
  input_directory <- gsub(
    "TestConfigs",
    "TestSpaces/geodynamic_spaces/raster",
    base_dir
  )
  output_directory <- file.path(tempdir(), "output")

  dirs <- evaluate_promise(prepare_directories(
    config_file = config_file,
  ...
  expect_equal(
    dirs$result$output_plots,
    file.path(output_directory, "TestConfig", "plots")
  )
})
...
test_that("wrong usage of create_input_config arguments", {
  # Config file does not exist
  expect_error(
    create_input_config(config_file = "this/file/is/a/lie.R"),
    "config file: this/file/is/a/lie.R does not exist"
  )

  # config_name is not a string
  expect_error(
    create_input_config(config_name = c("this", "is", "not", "a", "string")),
    "config_name must be length 1 or NULL, length 5 provided instead."
  )
  expect_error(
    create_input_config(config_name = list("this", "is", "not", "a", "string")),
    "config_name must be length 1 or NULL, length 5 provided instead."
  )
  expect_error(
    create_input_config(config_name = 13),
...
test_that("verify_config: config with missing values", {
  ...
  expect_message(verify_config(config), expected_message)
...
test_that("Correctly overwrites the file", {
  withr::with_tempdir({
    ...
    write_config_skeleton("config_skeleton.R")
    expect_warning(
      write_config_skeleton("config_skeleton.R", overwrite = FALSE),
...
test_that("boolean values of write_config_skeleton", {
  withr::with_tempdir({
    ...
    expect_false(
      suppressWarnings(write_config_skeleton(
        "config_skeleton.R",
        overwrite = FALSE
      ))
    )
...
test_that("bad usage of write_config_skeleton", {
  withr::with_tempdir({
    ...
    expect_error(
      write_config_skeleton(1),
      "file_path must be a character string containing the path to write the skeleton."
    )
    expect_error(
      write_config_skeleton(c("file", "path")),
      "file_path must be a character string containing the path to write the skeleton."
    )
    expect_error(
      write_config_skeleton(TRUE),
      "file_path must be a character string containing the path to write the skeleton."
    )
    expect_error(
      write_config_skeleton(data.frame(x = 1, y = 1)),
      "file_path must be a character string containing the path to write the skeleton."
    )
    ...
    expect_error(
      write_config_skeleton(file_path = "config_skeleton.R", overwrite = 1),
      "overwrite must be a logical value."
    )
    expect_error(
      write_config_skeleton(
        file_path = "config_skeleton.R",
        overwrite = data.frame(x = 1, y = 1)
      ),
      "overwrite must be a logical value."
    )
    expect_error(
    ...
    expect_error(
      write_config_skeleton(file_path = "config_skeleton.txt"),
      "File path must end with .R"
    )
    expect_error(
      write_config_skeleton(file_path = "config_skeleton.weirdstuff"),
      "File path must end with .R"
    )
    expect_error(
      write_config_skeleton(file_path = "config_skeleton"),
      "File path must end with .R"
    )
  })
})
