...
run_simulation <- function(
  ...
  for (ti in val$vars$steps) {
    ...
    # the within-site speciation loop
    val <- loop_within_site_speciation(val$config, val$data, val$vars)
    
    # note that we're not taking into account the new within-site species for the 
    # next loop (although they wouldn't be able to speciate or accumulate between-cluster)
    ...
    #   #------------------------------------------------------------------#
    val$vars$n_sp_alive <- sum(sapply(val$data$all_species, function(sp) {
    ...
    # abort conditions
    if (
      val$vars$n_sp_alive >= val$config$gen3sis$general$max_number_of_species
    ) {
  ...
  if (verbose >= 0 & val$vars$flag == "OK") {
    ...
    cat(
      "Simulation finished. Early abort due to exceeding max number of species"
    )
  } else if (verbose >= 0 & val$vars$flag == "max_number_coexisting_species") {
    cat(
      "Simulation finished. Early abort due to exceeding max number of co-occuring species"
    )
  }
  ...
  write.table(
    val$data$phy,
    file = file.path(val$config$directories$output, "phy.txt"),
    sep = "\t"
  )
  ...
  write_nex(
    phy = val$data$phy,
    label = "species",
    file.path(output_location = val$config$directories$output, "phy.nex")
  )
